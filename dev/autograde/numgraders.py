...
with specified tolerance. (use )
    change: 
        r = re.match('-?[0-9]+\.?[0-9]*j?', txt)
    to:
        r = re.match('-?[0-9]+\.?[0-9]+j?', txt)
    (notice that "5" will not match this pattern.)
    
    * further fix it for case of a single-digit number "5"
    change to:     
        r = re.match('-?[0-9]+(\.[0-9]+)?j?', txt) 
...
def check_answer(txtline1, txtline2, ntol):
    ...
    if N1 != N2: # different numbers of chucks
        # print('Debug: check_answer: diff # chucks')
        # Find difference
        N = min(N1, N2)
        pos = N
        for i in range(N):
    ...
    # print('Debug: check_answer: # chucks are matched')
    ...
    for i in range(N1):
        ...
        # print("Debug: check_answer: TL1[i]=", TL1[i])
        # print("Debug: check_answer: TL2[i]=", TL2[i])

        if is_number(TL1[i]) and is_number(TL2[i]):

            # print("* Debug: number case")
            ...
            # print("* Debug: text case")
            # print("** is_number({})".format(TL1[i]), is_number(TL1[i]))
            # print("** is_number({})".format(TL2[i]), is_number(TL2[i]))
...
if __name__ == '__main__':
    ...
    nums = ["5", "1234", "5.0", "-13", "-14.0", "18.9001", "-150"]
    nonnums = ["A1", "5.", ".5", "0A", "."]
